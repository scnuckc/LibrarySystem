  ...
  //创建用户数据库
    public static void newCustomer(String user) {
    	try{
            ...
            String sql = "CREATE TABLE "+user+"Customer (number VARCHAR(255) primary key,classname VARCHAR(255),name VARCHAR(255),dateoff VARCHAR(255));\n";    //要执行的SQl
            PreparedStatement stmts = conn.prepareStatement(sql);
            stmts.executeUpdate();

            sql = "create index number_classname on "+user+"Customer(number,classname);";    //要执行的SQl
    ...
    public static void prolongBook_Update(String classname,String number,String dateoff,String user) {
    	try{
            ...
            String sql="select number,classname from "+user+"Customer;\n";
            ResultSet rs = stmt.executeQuery(sql);//创建数据对象
            while (rs.next()){
                String s1=rs.getString(1);
                String s2=rs.getString(2);
                if(number.equals(s1) && classname.equals(s2))  {
                    sql = "begin;\n";    //要执行的SQL
                    PreparedStatement stmts_begin = conn.prepareStatement(sql);
                    stmts_begin.executeUpdate();
                    stmts_begin.close();
                    ...
                    sql = "commit;\n";    //提交事务
                    PreparedStatement stmts_commit = conn.prepareStatement(sql);
                    stmts_commit.executeUpdate();
                    stmts_commit.close();
                }
            }
  ...
  //查询个人图书信息
    ...
    public static boolean isExist_Table(String table) {
    	try{
            ...
            String sql = "show tables like '%"+table+"%';";    //要执行的SQL
            ResultSet rs = stmt.executeQuery(sql);//创建数据对象
                while (rs.next()){
